// v1.1 Banana Leaf Headless Controller
...
// BANANA LEAF: All FX forced active for testing
volatile bool isWhammyActive=true, isFrozen=true, isFeedbackActive=true, isHarmonizerMode=true, isSynthMode=true, isPadMode=true, isCapoMode=true, isChorusMode=true, isSwellMode=true, isVibratoMode=true, isVolumeMode=false, isPB2WiperMode=false;
...
bool commitDSPState() {
    ...
    // BANANA LEAF: Hard-lock all DSP states to TRUE for full load testing
    backBuffer->w = true;
    backBuffer->fz = true;
    backBuffer->fb = true;
    backBuffer->hr = true;
...
void switchEffectMode(int newMode) {
    ...
    isWhammyActive=true; isFrozen=true; isFeedbackActive=true; isHarmonizerMode=true; isSynthMode=true; isPadMode=true; isCapoMode=true; isChorusMode=true; isSwellMode=true; isVibratoMode=true;
    dspNeedsCommit = true;
...
void toggleSampleRate() {
    ...
    // BANANA LEAF: I2S clock remapping (MCLK = GPIO 39, BCLK = GPIO 40) to safe, free pins.
...
// BANANA LEAF: Headless Telemetry Task
void TelemetryTask(void * pvParameters) {
    for(;;) {
        ...
        if(!isScreenOff.load(std::memory_order_acquire)) {
            ...
            // BANANA LEAF: Accurately reflect all active effects in the DSP pipeline instead of just the UI Mode
...
void setup() {
    ...
    i2s_std_config_t stdConfig={ 
        .clk_cfg=I2S_STD_CLK_DEFAULT_CONFIG(currentSampleRate.load(std::memory_order_acquire)), 
        .slot_cfg=I2S_STD_PHILIPS_SLOT_DEFAULT_CONFIG(I2S_DATA_BIT_WIDTH_32BIT, I2S_SLOT_MODE_STEREO), 
        // BANANA LEAF MODIFICATION: Remapped MCLK to GPIO 39 and BCLK to GPIO 40
        .gpio_cfg={ .mclk=GPIO_NUM_39, .bclk=GPIO_NUM_40, .ws=GPIO_NUM_18, .dout=GPIO_NUM_16, .din=GPIO_NUM_17 } 
    };
    stdConfig.clk_cfg.mclk_multiple = I2S_MCLK_MULTIPLE_384; 
...
void loop() {
    ...
    if(settingsNeedSaving && (millis()-lastParameterChangeTime>2000)) { 
        ...
        // Save seamlessly in the background. No DSP parking or audio wiping needed.
        saveSettings();
    }
